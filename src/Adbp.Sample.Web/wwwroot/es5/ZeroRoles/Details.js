...
(function () {
    ...
    table_toadd.afterDraw = function () {
        $(".btn-member_toAdd").off("click").on("click", function () {
            ...
            var data = table_toadd.find($(this).closest("tr"));
...
(function () {
    var table_sysObjects = new abp.table.client("#table-details_sysObjects", {
        "order": [[1, "asc"]],
        "columnDefs": null,
        columns: [{ data: 'sysObjectName' }, { data: 'accessLevelStr' }]
    ...
    function initSetting(_ref) {
        var sysObjectName = _ref.sysObjectName,
            accessLevelInt = _ref.accessLevelInt;

        $("#SysObject_Name").val(sysObjectName);
        $("#sysObject-selected input[name=AccessLevel]").each(function () {
            var lev = eval($(this).val());
            var isChecked = (accessLevelInt & lev) > 0;
            $(this).prop("checked", isChecked);
        });
    }

    abp.event.on('adbp.formsubmitInitialized', function () {
        var s = $("#sysObject-selected_form").data("adbp_formSubmit");
