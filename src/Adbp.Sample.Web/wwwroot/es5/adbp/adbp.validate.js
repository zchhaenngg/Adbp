...
(function ($) {
    ...
    function isTextArea(selector) {
        ...
        return $selector[0].nodeName.toUpperCase() === "TEXTAREA" || $selector.attr("type").toUpperCase() === "TEXTAREA";
    }
    ...
    function validating(value, _ref) {
        var name = _ref.name,
            required = _ref.r,
            minL = _ref.minL,
            maxL = _ref.maxL,
        ...
        if (bReg) {
            var r = abp.validate.regexps[bReg];
            if (r === undefined) {
                throw new "bReg is not undefined"();
            }
    ...
    abp.validate = function (container) {
        ...
        for (var i = 0; i < params.length; i++) {
            ...
            var $e = $(container).find("[name=" + name + "]");
            var opts = abp.validate.getOptions($e);
    ...
    abp.validate.getOptions = function (selector) {
        ...
        var optionStr = $(selector).data("vd");
        if (optionStr) {
            var opts2 = eval("(" + optionStr + ")");
