...
(function ($) {
    ...
    let libs = {
        ...
        allSelected: function (dt) {//if no selected return []
            libs.checkDataTable(dt);

            return dt.rows(".selected").data().toArray();
        },
        clearSelected: function (dt) {
            libs.checkDataTable(dt);

            dt.rows().deselect();
        },
        find: function (dt, rowSelector) {
            libs.checkDataTable(dt);
    ...
    abp.table.client = (function () {
        ...
        ClientTable.prototype.show = function () {
            ...
            if (defered !== undefined && defered.done) {
                abp.ui.setBusy();
                return defered.done(function ({ items }) {
                    abp.ui.clearBusy();
        ...
        ClientTable.prototype._callContacts = function () {
            ...
            let _this = this;
            let drawContacts = this._contacts.filter(x => x.type.includes('draw.dt'));
            let selectContacts = this._contacts.filter(x => x.type.includes('select.dt'));
            let deselectContacts = this._contacts.filter(x => x.type.includes('deselect.dt'));
    ...
    abp.table.server = (function () {
        function ServerTable(selector, options) {
            ...
            $(this._selector).on('error.dt', function (e, settings, techNote, message) {
                abp.notify.error("please check your permission(table)! or " + message);
            });
        }
        ...
        */
        ...
        ServerTable.prototype._render = function () {
            ...
            return $.Deferred(function ($dfd) {
                abp.ui.setBusy();
                let options = $.extend({}, _this._options, {
