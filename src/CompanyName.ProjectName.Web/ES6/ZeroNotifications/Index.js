(function () {
    ...
    window.table = new abp.table.client("#table-notifications", {
        ...
        columns: [
            ...
            { data: 'description' },
            {
                data: 'isSubscribed', render: function (data, type, full, meta) {
                    if (data == null) {
                        return "";
                    }
                    else if (data) {
    ...
    $("#btn-notification_edit").on("click", function () {
        ...
        if (row.isSubscribed) {
            abp.message.confirm('', "确认取消订阅！").done((value) => {
                if (value) {
            ...
            abp.message.confirm('', "确认订阅！").done((value) => {
                if (value) {
...
(function () {
    ...
    let tableRead = new abp.table.client("#table-notifications_read", {
        "order": [[1, "desc"]],
        "select": 'multi',//multi
        columns: [
            {
            ...
            { data: 'description' },
            {
                data: 'severityStr', render: function (data, type, full, meta) {
                    return abp.localization.localize("ENUM_NotificationSeverity_" + data, "AdbpZero");
                }
            },
            {
                data: 'stateStr', render: function (data, type, full, meta) {
    ...
    $("#btn-notification_read").on("click", function () {
        let ids = tableRead.allSelected().map(x => x.id);
